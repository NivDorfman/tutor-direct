...
* **למורים:** מאפשרת ניהול פרופיל מקצועי מורחב, קביעת מחיר עצמאית, ניהול חלונות זמן פנויים (Time Slots), צפייה בשיעורים שהוזמנו, מענה לתלמידים, וצפייה בפרופילי מורים עמיתים להשראה (תוך חסימת צ'אט או ביקורות מטעמי טוהר דירוג).
...
5. **Testing & QA:** סוויטת בדיקות אוטומטית מלאה באמצעות **Vitest** (38 בדיקות יחידה ואינטגרציה).
...
│   │   ├── reset-password/
│   │   │   └── page.tsx                # עמוד שחזור ואיפוס סיסמה ייעודי
...
│   │   ├── LiveLessonModal.tsx         # חדר שיעור חי אינטראקטיבי ולוח שיתופי
│   │   ├── ManageSlotsModal.tsx        # ניהול חלונות זמן פנויים (למורים)
│   │   ├── MyBookingsModal.tsx         # ניהול שיעורים והזמנות קרובות
│   │   ├── ResetPasswordModal.tsx      # מודל איפוס סיסמה מבוסס OTP
│   │   ├── StudyMaterialsSection.tsx   # ניהול והורדת חומרי לימוד וסיכומים
...
│   │   ├── TutorDirectLogo.tsx         # לוגו וקטורי מותאם מותג
│   │   └── UserProfileModal.tsx        # עדכון פרופיל משתמש, סיסמה ועריכת פרטי מורה
│   ├── lib/                            # לוגיקה עסקית ואינטגרציות
│   │   ├── businessLogic.ts            # ולידציות, אלגוריתמי סינון ותיאום
│   │   ├── i18n.ts                     # מנגנון תמיכה רב-לשונית (עברית/אנגלית)
│   │   ├── slotUtils.ts                # עזרי תאריכים וחלונות זמן
│   │   ├── storageUtils.ts             # עזרי אחסון וניהול חומרי לימוד
