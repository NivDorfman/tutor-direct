...
export default function App() {
  ...
  const loadDataFromSupabase = useCallback(async () => {
      try {
        ...
        const [tutorsRes, usersRes, slotsRes, msgsRes] = await Promise.all([
        ...
        // 1. Process INITIAL_TUTORS first to maintain standard catalog order & rich content
        INITIAL_TUTORS.forEach((it: Tutor) => {
          const itEmail = (it.email || '').toLowerCase().trim();
          const itName = (it.name || '').trim();

          // Match with users table from Supabase
          const userMatch = usersData.find((u: any) => {
            const uEmail = (u.email || '').toLowerCase().trim();
            const uName = (u.name || '').trim();
            const uId = u.id;
            return (
              (itEmail && uEmail && itEmail === uEmail) ||
              (itName && uName && (itName.toLowerCase() === uName.toLowerCase())) ||
              (uId && uId === it.id) ||
              (u.tutor_profile_id && u.tutor_profile_id === it.id)
            );
          });

          // Match with tutors table from Supabase
          const dbTutorMatch = tutorsData.find((t: any) => {
            return (
              t.id === it.id ||
              (userMatch && t.id === userMatch.id) ||
              (t.email && itEmail && t.email.toLowerCase().trim() === itEmail) ||
              (t.name && itName && t.name.trim().toLowerCase() === itName.toLowerCase())
            );
          });

          // Resolved avatar: Supabase user avatar has top priority across all devices
          const resolvedAvatar = 
            userMatch?.avatar || 
            userMatch?.avatar_url || 
            userMatch?.avatarUrl || 
            dbTutorMatch?.avatar ||
            dbTutorMatch?.avatar_url || 
            dbTutorMatch?.avatarUrl || 
            it.avatarUrl;

          // Resolved slots
          const tutorSlots = slotsData
            .filter((s: any) => s.tutor_id === it.id || (userMatch && s.tutor_id === userMatch.id) || (dbTutorMatch && s.tutor_id === dbTutorMatch.id))
            .map((s: any) => {
          ...
          const resolvedReviews = resolveReviewsForTutor(it.id, it.email, it.name, it.reviews || []);

          const merged: Tutor = {
            ...it,
            id: it.id,
            name: userMatch?.name || dbTutorMatch?.name || it.name,
            email: userMatch?.email || dbTutorMatch?.email || it.email,
            phone: userMatch?.phone || dbTutorMatch?.phone || it.phone,
            subject: dbTutorMatch?.subject || it.subject,
            price: Number(dbTutorMatch?.price ?? it.price),
            bio: dbTutorMatch?.bio || it.bio,
            education: dbTutorMatch?.education || it.education,
            experience: dbTutorMatch?.experience || it.experience,
            levels: dbTutorMatch?.levels ? (Array.isArray(dbTutorMatch.levels) ? dbTutorMatch.levels.join(', ') : String(dbTutorMatch.levels)) : it.levels,
            avatarUrl: resolvedAvatar,
            availableSlots: tutorSlots.length > 0 ? tutorSlots : it.availableSlots,
            rating: resolvedReviews.rating,
            reviews: resolvedReviews.reviews,
            studyMaterials: it.studyMaterials || []
          };

          combinedTutors.push(merged);
          if (it.id) seenIds.add(it.id);
          if (userMatch?.id) seenIds.add(userMatch.id);
          if (dbTutorMatch?.id) seenIds.add(dbTutorMatch.id);
          if (itEmail) seenEmails.add(itEmail);
          if (userMatch?.email) seenEmails.add(userMatch.email.toLowerCase().trim());
          if (itName) seenNames.add(itName.toLowerCase());
          if (userMatch?.name) seenNames.add(userMatch.name.trim().toLowerCase());
        });

        // 2. Process non-initial tutors from tutorsData (custom registered tutors in Supabase)
        tutorsData.forEach((t: any) => {
          ...
          if (tName && seenNames.has(tName.toLowerCase()) && tName !== 'מורה מוסמך') return;
          ...
          const customTutor: Tutor = {
            ...
            levels: t.levels ? (Array.isArray(t.levels) ? t.levels.join(', ') : String(t.levels)) : 'תיכון',
            avatarUrl: resolvedAvatar,
            availableSlots: tutorSlots.length > 0 ? tutorSlots : generateDefaultSlots(t.id),
        ...
        // 3. Process any users with role === 'teacher' not yet included
        ...
        teacherUsers.forEach((u: any) => {
          ...
          if (uName && seenNames.has(uName.toLowerCase()) && uName !== 'מורה מוסמך') return;
          ...
          const customTutor: Tutor = {
            ...
            levels: 'תיכון',
            avatarUrl: resolvedAvatar,
            availableSlots: tutorSlots.length > 0 ? tutorSlots : generateDefaultSlots(u.id),
        ...
        // Sync logged-in user avatar from Supabase in real-time
        try {
          const rawStoredUser = localStorage.getItem('current_user');
          if (rawStoredUser) {
            const parsedCurrentUser = JSON.parse(rawStoredUser);
            if (parsedCurrentUser?.email || parsedCurrentUser?.id) {
              const currentDbUser = usersData.find((u: any) => 
                (parsedCurrentUser.id && u.id === parsedCurrentUser.id) || 
                (parsedCurrentUser.email && u.email && u.email.toLowerCase() === parsedCurrentUser.email.toLowerCase())
              );
              if (currentDbUser) {
                const freshAvatar = currentDbUser.avatar || currentDbUser.avatar_url || currentDbUser.avatarUrl;
                if (freshAvatar && freshAvatar !== parsedCurrentUser.avatarUrl) {
                  const updatedCurrent = { ...parsedCurrentUser, avatarUrl: freshAvatar };
        ...
        if (combinedTutors.length > 0) {
          setTutors(combinedTutors);
          try {
            localStorage.setItem('private_tutors', JSON.stringify(combinedTutors));
          } catch (e) {}
        ...
        // Map bookings from BOTH:
        // 1) Slots table where is_booked === true
        const mappedBookings: Booking[] = [];
        const bookedSlots = slotsData.filter((s: any) => s.is_booked === true);
        ...
        bookedSlots.forEach((row: any) => {
          ...
          // Check if there is a matching booking message to get rich student details
          ...
          // If no student is found at all (e.g. orphan slot without student), skip adding a phantom booking
          if (!row.student_id && !matchingMsg && !studentUser) {
            return;
          }
        ...
        // 2) Parse bookings from chat messages and merge without duplicating
        bookingMsgs.forEach((msg: any) => {
          ...
          const day = dayMatch ? dayMatch[1].trim() : 'יום ראשון';
          const time = timeMatch ? timeMatch[1].trim() : '16:00 - 17:00';
          const note = noteMatch ? noteMatch[1].trim() : '';

          let studentId = msg.sender_id;
          let tutorId = msg.receiver_id;

          let studentUser = usersData.find((u: any) => u.id === studentId);
          let tutorUser = usersData.find((u: any) => u.id === tutorId);

          if (studentUser?.role === 'teacher' && tutorUser?.role === 'student') {
            const temp = studentUser;
            studentUser = tutorUser;
            tutorUser = temp;
            studentId = studentUser?.id || msg.receiver_id;
            tutorId = tutorUser?.id || msg.sender_id;
          }

          const tutorInfo = tutorsData.find((t: any) => t.id === tutorId);
          const studentName = (nameMatch ? nameMatch[1].trim() : '') ||
            (studentUser?.name && studentUser.name.trim() !== '' && studentUser.name !== 'תלמיד'
              ? studentUser.name
              : (studentUser?.email ? studentUser.email.split('@')[0] : 'תלמיד'));
          const studentEmail = (emailMatch ? emailMatch[1].trim() : '') || studentUser?.email || studentId || '';

          // Look for an existing booking to merge with
          const existingBooking = mappedBookings.find(b => 
            b.id === msg.id ||
            b.slot.id === msg.id ||
            (isSameSlot(b.slot, { day, time }) && (
              (tutorId && b.tutorId && b.tutorId === tutorId) ||
              (studentId && (b as any).studentId && (b as any).studentId === studentId) ||
              (studentEmail && b.studentEmail && b.studentEmail.toLowerCase() === studentEmail.toLowerCase())
            ))
          );

          if (existingBooking) {
            // Update existing booking with rich info if missing
            if ((!existingBooking.studentName || existingBooking.studentName === 'תלמיד') && studentName) {
              existingBooking.studentName = studentName;
            }
            if (!existingBooking.studentEmail && studentEmail) {
              existingBooking.studentEmail = studentEmail;
            }
            if (!existingBooking.note && note) {
            ...
            mappedBookings.push({
              ...
              note,
              createdAt: msg.created_at ? new Date(msg.created_at).toLocaleDateString('he-IL') : new Date().toLocaleDateString('he-IL'),
              status: 'ממתין'
            });
          }
        });

        // 3) Parse and apply real-time status updates from chat messages across all devices
        msgsData.forEach((m: any) => {
          if (!m.text) return;
          ...
          const isCancelled = m.text.includes('השיעור בוטל') || m.text.includes('STATUS_UPDATE:CANCELLED') || m.text.includes('בוטל על ידי');

          if (!isApproved && !isCompleted && !isCancelled && !isStudentConfirmed && !isTeacherConfirmed) return;
          ...
          mappedBookings.forEach(mb => {
            ...
            const matchBySlotAndParticipants = sameSlot && (
              mb.tutorId === p1 || mb.tutorId === p2 ||
              (mb as any).studentId === p1 || (mb as any).studentId === p2 ||
              (currentUser?.id && (mb.tutorId === currentUser.id || (mb as any).studentId === currentUser.id))
            );

            if (matchById || matchBySlotAndParticipants) {
              if (isCompleted) {
                mb.status = 'הושלם';
        ...
        // Merge with local booking notes & statuses if any, and deduplicate cleanly
        ...
        try {
          ...
          if (localBookingsRaw) {
            ...
            if (Array.isArray(localList) && localList.length > 0) {
              combinedBookings.push(...localList);
            }
          }
        } catch (e) {}
  ...
  const saveTutors = async (updatedTutors: Tutor[], specificTutorToSave?: Tutor) => {
    ...
    localStorage.setItem('private_tutors', JSON.stringify(updatedTutors));
    try {
  ...
  const handleUpdateSlots = async (tutorId: string, updatedSlots: TimeSlot[]) => {
    // Update the tutor's slots in the tutors list
    const updatedTutors = tutors.map(t => {
      if (t.id === tutorId) {
    ...
    const originalTutor = tutors.find(t => t.id === tutorId);
    if (originalTutor) {
      const removedSlots = originalTutor.availableSlots.filter(
        origSlot => !updatedSlots.some(newSlot => newSlot.id === origSlot.id)
      );
      
      if (removedSlots.length > 0) {
        const updatedBookings = bookings.map(b => {
          if (b.tutorId === tutorId && b.status !== 'בוטל' && removedSlots.some(s => s.day === b.slot.day && s.time === b.slot.time)) {
    ...
    // Sync to Supabase 'slots' table
    try {
      if (isValidUuid(tutorId)) {
        await supabase.from('slots').delete().eq('tutor_id', tutorId);
  ...
  const handleBookLesson = async (
    ...
    // Save Booking in React state & local storage
    const updatedBookings = deduplicateBookings([newBooking, ...bookings]);
    setBookings(updatedBookings);
    try {
      localStorage.setItem('tutor_bookings', JSON.stringify(updatedBookings));
    } catch (e) {}
      ...
      if (targetTutorUuid) {
        ...
        if (isValidUuid(slot.id)) {
          const { data: updated } = await supabase
            .from('slots')
            .update({ is_booked: true, student_id: studentUuid })
            .eq('id', slot.id)
            .select();
    ...
    try {
      if (isValidUuid(targetBooking.slot.id)) {
        await supabase
          ...
          .eq('id', targetBooking.slot.id);
      } else if (isValidUuid(targetBooking.tutorId)) {
        const iso = dayTimeToIso(targetBooking.slot.day, targetBooking.slot.time);
        await supabase
          .from('slots')
          .update({ is_booked: false, student_id: null })
          .eq('tutor_id', targetBooking.tutorId)
          .eq('datetime', iso);
      }
      ...
      const senderId = currentUser?.id && isValidUuid(currentUser.id) 
        ? currentUser.id 
        : (currentUser?.role === 'teacher' ? teacherId : studentId);
