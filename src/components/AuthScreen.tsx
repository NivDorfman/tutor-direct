...
import { validateRegistration, normalizePhoneNumber, validateLoginInput, matchUserByIdentifier } from '../lib/businessLogic';
...
export const AuthScreen: React.FC<AuthScreenProps> = ({ 
  ...
  // Login form state (supports username or email)
  const [loginIdentifier, setLoginIdentifier] = useState(recoveryUserEmail || '');
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    const rawIdentifier = (loginIdentifier || email).trim();
    ...
    try {
      // 1. Direct query against Supabase 'users' table (by email or username)
      ...
      if (rawIdentifier.includes('@')) {
        ...
        // Fallback: try email match in case of atypical format
        if (!userRow) {
      ...
      if (userRow) {
        ...
        if (dbPassword) {
          ...
          if (stored) {
            try {
              ...
              const foundLocal = users.find((u: any) => 
                (u.id && u.id === userRow.id) ||
                (u.email && u.email.toLowerCase() === userRow.email?.toLowerCase()) ||
                (u.name && u.name.trim().toLowerCase() === userRow.name?.trim().toLowerCase())
              );
        ...
        const authenticatedUser = {
          id: userRow.id,
          name: userRow.name || userRow.email?.split('@')[0] || rawIdentifier,
          email: userRow.email || (rawIdentifier.includes('@') ? rawIdentifier : `${cleanIdentifier}@example.com`),
    ...
    // 2. Local fallback for offline / demo users (matching name or email)
    const storedUsers = localStorage.getItem('registered_users');
    if (storedUsers) {
      try {
        const users: any[] = JSON.parse(storedUsers);
        const matchedLocalUser = matchUserByIdentifier<any>(users, rawIdentifier);
        if (matchedLocalUser) {
    ...
    // 3. Check pre-seeded demo tutors in existingTutors (by name or email)
    const matchedTutor = matchUserByIdentifier(existingTutors, rawIdentifier);
    if (matchedTutor) {
        ...
        <div className="bg-white py-8 px-6 sm:px-10 border border-slate-200 rounded shadow-sm space-y-6">
          ...
          {authMode === 'login' && (
            <form onSubmit={handleLoginSubmit} className="space-y-4" id="login-form">
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">שם משתמש או כתובת אימייל</label>
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (loginIdentifier.includes('@')) {
                        initialReset = loginIdentifier.trim();
                      } else if (loginIdentifier.trim()) {
                        const stored = localStorage.getItem('registered_users');
                        if (stored) {
                          try {
                            const users = JSON.parse(stored);
                            const matched = users.find((u: any) => u.name && u.name.trim().toLowerCase() === loginIdentifier.trim().toLowerCase());
                            if (matched?.email) initialReset = matched.email;
                          } catch (e) {}
                        }
                      }
                      setResetEmail(initialReset);
                      setError('');
                      setSuccessMsg('');
                      setResetStep('email');
                    }}
