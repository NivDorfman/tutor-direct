...
import { normalizeTime } from '../lib/slotUtils';
...
export const ManageSlotsModal: React.FC<ManageSlotsModalProps> = ({ tutor, onUpdateSlots, onClose }) => {
  ...
  const handleAddSlot = (e?: React.FormEvent) => {
    ...
    const normalizedNewTime = normalizeTime(newTime.trim());
    ...
    const isDuplicate = currentSlots.some(
      (slot) => slot.day === newDay && normalizeTime(slot.time) === normalizedNewTime
    );
                        ...
                        {slotsForDay.map((slot) => (
                          ...
                          >
                            <div className="flex items-center gap-2">
                              ...
                              <span className="font-bold text-slate-700 ltr inline-block" dir="ltr">{slot.time}</span>
                              {slot.isBooked && (
