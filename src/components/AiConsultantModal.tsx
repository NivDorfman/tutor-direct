...
export function AiConsultantModal({ 
  ...
  const handleSendMessage = async (textToSend: string) => {
    ...
    // Abort controller with 30s timeout as client-side safety net
    ...
    try {
      ...
      if (!response.ok || !data) {
        const errorMsg = data?.error || (isRtl ? 'עומס זמני בשרת ה-AI. ניתן ללחוץ על "נסה שוב" או להמשיך לשאול.' : 'Temporary AI server demand. You can click "Retry" or ask another question.');
        setError(errorMsg);
      ...
      setError(
        isTimeout 
          ? (isRtl ? 'הבקשה לקחה זמן רב עקב עומס זמני. לחץ על "נסה שוב".' : 'Request timed out due to high demand. Please click "Retry".')
          : (isRtl ? 'שגיאה זמנית בחיבור ליועץ ה-AI. אנא לחץ על "נסה שוב".' : 'Temporary connection error to AI advisor. Please click "Retry".')
      );
      setLastFailedText(textToSend);
    } finally {
      setIsLoading(false);
  ...
  const getTutorsInLine = (lineText: string, alreadyRendered: Set<string>): Tutor[] => {
    if (!lineText) return [];
    const matched: Tutor[] = [];
    tutors.forEach(t => {
      if (!t.name || t.name.trim().length < 2) return;
      if (alreadyRendered.has(t.id)) return;

      const trimmedName = t.name.trim();
      const firstName = trimmedName.split(/\s+/)[0];

      // Match exact whole name or first name boundary in Unicode (Hebrew/English)
      try {
        const fullRegex = new RegExp(`(?:^|[^\\p{L}\\p{N}])` + escapeRegExp(trimmedName) + `(?:[^\\p{L}\\p{N}]|$)`, 'u');
        const firstRegex = firstName.length >= 2 
          ? new RegExp(`(?:^|[^\\p{L}\\p{N}])` + escapeRegExp(firstName) + `(?:[^\\p{L}\\p{N}]|$)`, 'u')
          : null;

        if (fullRegex.test(lineText) || (firstRegex && firstRegex.test(lineText))) {
          matched.push(t);
          alreadyRendered.add(t.id);
        }
      } catch (e) {
        // Fallback for environments where unicode property escapes might differ
        if (lineText.includes(trimmedName) || (firstName.length >= 2 && lineText.includes(firstName))) {
          matched.push(t);
          alreadyRendered.add(t.id);
        }
      }
  ...
  const renderMessageText = (text: string) => {
    ...
    const alreadyRenderedTutorIds = new Set<string>();

    return lines.map((line, i) => {
      const trimmed = line.trim();
      const tutorsInThisLine = getTutorsInLine(line, alreadyRenderedTutorIds);

      if (trimmed.startsWith('### ') || trimmed.startsWith('## ') || trimmed.startsWith('# ')) {
        const title = trimmed.replace(/^#+\s*/, '');
        return (
          <div key={i} className="mt-3 mb-1.5 pb-1 border-b border-indigo-100">
            <h4 className="text-xs font-black text-indigo-950">
              {title}
            </h4>
            {tutorsInThisLine.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-1.5">
                {tutorsInThisLine.map(tut => (
                  <button
                    key={tut.id}
                    type="button"
                    onClick={() => {
                    ...
                    className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-indigo-600 hover:bg-indigo-700 active:scale-95 text-white rounded-lg text-[11px] font-extrabold transition-all shadow-xs cursor-pointer"
                  >
                    <User className="w-3 h-3 text-indigo-200" />
                    <span>{isRtl ? `צפה בפרופיל (${tut.name})` : `View Profile (${tut.name})`}</span>
      ...
      if (trimmed.startsWith('* ') || trimmed.startsWith('- ') || trimmed.startsWith('• ')) {
        ...
        return (
          <div key={i} className="flex flex-col my-1 text-slate-800 pr-1">
            ...
            {tutorsInThisLine.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mr-3 ml-3 mt-1.5 mb-1">
                {tutorsInThisLine.map(tut => (
                  <button
                    key={tut.id}
                    type="button"
                  ...
                  >
                    <User className="w-3 h-3 text-indigo-200" />
                    <span>{isRtl ? `צפה בפרופיל של ${tut.name}` : `View ${tut.name}'s Profile`}</span>
                    <ArrowLeft className={`w-3 h-3 ${!isRtl ? 'rotate-180' : ''}`} />
                  </button>
                ))}
              </div>
      ...
      if (numMatch) {
        return (
          <div key={i} className="flex flex-col my-1.5 text-slate-800 pr-1">
            ...
            {tutorsInThisLine.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mr-6 ml-6 mt-1.5 mb-1">
                {tutorsInThisLine.map(tut => (
                  <button
                    key={tut.id}
                    type="button"
      ...
      return (
        <div key={i} className="leading-relaxed mb-1.5 last:mb-0 text-slate-800">
          <p>{formatInlineStyles(line)}</p>
          {tutorsInThisLine.length > 0 && (
  ...
  const quickPrompts = isRtl ? [
    { text: "אני רוצה מורה שמלמד כיתה ט", icon: BookOpen },
    { text: "איזה מורה מומלץ למתמטיקה 5 יחידות?", icon: BookOpen },
    { text: "מי פנוי ביום שלישי בשעה 16:00?", icon: Sparkles },
    { text: "יש מורים לאנגלית בפחות מ-130 ש״ח לשעה?", icon: DollarSign },
  ] : [
    { text: "I want a tutor who teaches 9th grade (כיתה ט)", icon: BookOpen },
    { text: "Which tutor is recommended for advanced Math?", icon: BookOpen },
    { text: "Who is available on Tuesday at 16:00?", icon: Sparkles },
    { text: "Are there English tutors under 130 ₪/hour?", icon: DollarSign },
  ];

  return (
    <div 
      className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center z-50 p-1.5 sm:p-4 font-sans" 
      dir={isRtl ? 'rtl' : 'ltr'}
      onClick={(e) => {
    ...
    >
      <div 
        ...
        className="relative bg-slate-50 rounded-2xl max-w-3xl w-full h-[95vh] sm:h-[84vh] flex flex-col overflow-hidden shadow-2xl border border-slate-200"
      >
        {/* Header */}
        <div className="bg-gradient-to-r from-indigo-700 via-indigo-800 to-slate-900 text-white px-3 sm:px-6 py-2.5 sm:py-3.5 flex items-center justify-between shrink-0 shadow-md gap-2">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1.5 flex-wrap">
                <h2 className="text-xs sm:text-base font-black tracking-tight truncate">{t.aiConsultantTitle}</h2>
                <span className="bg-amber-400/20 text-amber-300 text-[9px] sm:text-[10px] font-black px-1.5 py-0.5 rounded-full border border-amber-300/30 uppercase tracking-wider shrink-0">
                  Gemini AI
                </span>
              </div>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            <button
              ...
              className="p-1.5 sm:px-3 sm:py-1.5 bg-white/10 hover:bg-white/20 active:scale-95 border border-white/15 text-white rounded-xl sm:rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer"
            ...
            <button 
              ...
              id="btn-ai-close"
              onClick={onClose}
              className="p-1.5 sm:p-2 bg-white/20 hover:bg-white/30 active:scale-95 rounded-xl sm:rounded-lg text-white border border-white/30 transition-all flex items-center justify-center shadow-xs cursor-pointer"
              title={t.close || (isRtl ? "סגור" : "Close")}
              aria-label={t.close || (isRtl ? "סגור" : "Close")}
            >
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4 sm:space-y-5 bg-slate-50/50">
          ...
          {error && (
            <div className="p-3.5 bg-rose-50 border border-rose-200 rounded-xl text-rose-700 text-xs flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 shadow-xs">
              <div className="flex items-start gap-2.5">
                <AlertCircle className="w-4 h-4 text-rose-600 shrink-0 mt-0.5" />
                <div>
                  <p className="font-extrabold">{isRtl ? 'עומס זמני בשירות' : 'Temporary High Demand'}</p>
              ...
              {lastFailedText && (
                <button
                  ...
                  className="self-end sm:self-auto px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 active:scale-95 text-white font-bold rounded-lg text-xs flex items-center gap-1.5 transition-all shadow-xs cursor-pointer shrink-0"
                >
                  <RotateCcw className="w-3.5 h-3.5" />
                  <span>{isRtl ? 'נסה שוב כעת' : 'Retry Now'}</span>
                </button>
              )}
            </div>
          )}
