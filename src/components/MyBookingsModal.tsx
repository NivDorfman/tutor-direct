...
export const MyBookingsModal: React.FC<MyBookingsModalProps> = ({
  ...
  // Filter and deduplicate bookings based on user role
  const filteredBookings = deduplicateBookings(bookings.filter(booking => {
    if (currentUser.role === 'teacher') {
      ...
      return Boolean(matchId || matchProfileId || matchEmail || matchName);
    } else {
                    ...
                    {booking.status === 'הושלם' && (() => {
                      ...
                      return (
                        <div className="pt-2 border-t border-slate-100 mt-2 space-y-3">
                          ...
                          {/* Already Submitted Review Box (Locked - 1 Review Only) */}
                            ...
                            <div className="bg-emerald-50/90 border border-emerald-200 rounded-lg p-3.5 space-y-2 text-right">
                              <div className="flex items-center justify-between">
                                ...
                                <span className="text-[11px] font-bold text-emerald-700 bg-emerald-100/80 px-2 py-0.5 rounded border border-emerald-200">
                                  {language === 'he' ? 'חוות דעת אחת בלבד ✓' : '1 review only ✓'}
                                </span>
                              </div>
                          ...
                          {currentUser.role === 'student' && !submittedReview && isAlreadyReviewedForTutor && (
                            <div className="bg-slate-100/90 border border-slate-200 rounded-lg p-3 text-right flex items-center justify-between text-xs text-slate-600">
                            ...
                            >
                              <div className="flex items-center justify-between border-b border-indigo-100 pb-2">
                                <div className="flex items-center gap-2 text-xs font-bold text-indigo-950">
                                  ...
                                  <span>{language === 'he' ? `כתיבת חוות דעת ודירוג עבור ${booking.tutorName}` : `Write Review for ${booking.tutorName}`}</span>
                                </div>
                                <button
                                  type="button"
