...
export const ChatWidget: React.FC<ChatWidgetProps> = ({
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    let targetPartnerUuid = activeConvId;
    if (!isValidUuid(targetPartnerUuid)) {
      const matched = usersMap.get(targetPartnerUuid) || 
        (activeConv?.studentEmail ? usersMap.get(activeConv.studentEmail.toLowerCase()) : null) ||
        (activeConv?.tutorEmail ? usersMap.get(activeConv.tutorEmail.toLowerCase()) : null);
      if (matched && isValidUuid(matched.id)) {
        targetPartnerUuid = matched.id;
      } else if (activeConv?.studentEmail) {
        const resolved = await resolveUserUuid({
          name: activeConv.studentName || 'תלמיד',
          email: activeConv.studentEmail,
          role: 'student'
        });
        if (resolved) targetPartnerUuid = resolved;
      }
    }

    if (!targetPartnerUuid || !isValidUuid(targetPartnerUuid)) {
      console.error("Target partner ID is not a valid UUID:", targetPartnerUuid);
      return;
    }

    if (isTargetTeacher) {
      console.warn("Teachers are not allowed to send messages to other teachers.");
      return;
    }
  ...
  // Find the tutor associated with active conversation (only relevant for students booking lessons)
  const activeTutor = (currentUser.role === 'student' && activeConv)
    ? (tutors.find(t => 
        (t.id !== myUuid && t.id !== currentUser.id && (!t.email || !currentUser.email || t.email.toLowerCase() !== currentUser.email.toLowerCase())) &&
        (t.id === activeConv.tutorId || t.id === activeConv.id || (activeConv.tutorEmail && t.email?.toLowerCase() === activeConv.tutorEmail?.toLowerCase()))
      ) || (initialTutorToChat && (initialTutorToChat.id === activeConv.tutorId || initialTutorToChat.id === activeConv.id) ? initialTutorToChat : null))
    : null;

  // Identify partner user record from active conversation
  const partnerId = activeConvId;
  const partnerEmail = activeConv 
    ? (currentUser.role === 'teacher' ? activeConv.studentEmail : activeConv.tutorEmail)
    : undefined;

  const partnerUser = partnerId 
    ? (usersMap.get(partnerId) || (partnerEmail ? usersMap.get(partnerEmail.toLowerCase()) : null))
    : null;

  // Peer teacher check: Only block if current user is a teacher AND the target partner is explicitly another teacher.
  // Teachers can and must be able to freely communicate with their students!
  const isTargetTeacher = Boolean(
    currentUser.role === 'teacher' &&
    partnerId &&
    partnerId !== myUuid &&
    partnerId !== currentUser.id &&
    (
      // 1. Partner has role === 'teacher' in users table (and is not current user)
      (partnerUser && partnerUser.role === 'teacher' && partnerUser.id !== myUuid && (!partnerUser.email || !currentUser.email || partnerUser.email.toLowerCase() !== currentUser.email.toLowerCase())) ||
      // 2. Partner is another tutor in the tutors catalog (and is not current user)
      tutors.some(t => 
        (t.id !== myUuid && t.id !== currentUser.id && (!t.email || !currentUser.email || t.email.toLowerCase() !== currentUser.email.toLowerCase())) &&
        (
          t.id === partnerId || 
          (partnerEmail && t.email?.toLowerCase() === partnerEmail.toLowerCase()) ||
          (partnerUser?.email && t.email?.toLowerCase() === partnerUser.email.toLowerCase())
        )
      )
    )
  );
          ...
          {activeConv ? (
            <>
              ...
              <div className="p-3 bg-white border-t border-slate-200 shrink-0">
                {isTargetTeacher ? (
                  <div className="text-center py-2.5 px-3 text-xs text-slate-500 bg-slate-50 rounded-xl border border-slate-200">
                    🔒 מורים אינם מורשים לשלוח הודעות למורים עמיתים במערכת
                  </div>
                ) : (
