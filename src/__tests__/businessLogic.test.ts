...
import { validateRegistration, normalizePhoneNumber, filterTutors, simulateBooking, validateReviewEligibility, RegistrationData, FilterParams, BookingInput, ReviewEligibilityInput } from '../lib/businessLogic';
...
describe('TutorDirect Business Logic & Unit Tests', () => {
  ...
  describe('validateReviewEligibility (אימות הרשאת כתיבת חוות דעת לאחר שיעור)', () => {
    ...
    it('should reject review if student already reviewed this tutor', () => {
      const result = validateReviewEligibility({
      ...
      expect(result.eligible).toBe(false);
      expect(result.reason).toBe('כבר נתת חוות דעת למורה זה בעבר');
    });
